...
#pred_folders = ['res34cls2_0_tuned', 'res34cls2_1_tuned', 'res34cls2_2_tuned'] + ['res50cls_cce_0_tuned', 'res50cls_cce_1_tuned', 'res50cls_cce_2_tuned'] + ['se154cls_0_tuned', 'se154cls_1_tuned', 'se154cls_2_tuned']
#pred_coefs = [1.0] * 12
...
def process_image(f):
    ...
    #     #msk2 = cv2.imread(path.join(d, f.replace('_', '_part2')), cv2.IMREAD_UNCHANGED)
    ...
    msk_loc = (255 * ((loc_preds > _thr[0]) | ((loc_preds > _thr[1]) | ((loc_preds > _thr[2]))))).astype('uint8')
...
if __name__ == '__main__':
    ...
    all_files = []
    for f in tqdm(sorted(listdir(loc_folders[0]))):
        if '.png' in f:
            all_files.append(f)
